...
const portals = [
  {
    ...
    icon: (
      <svg className="w-10 h-10 text-tenant-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    icon: (
      <svg className="w-10 h-10 text-persona-teacher" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    persona: 'management' as const,
    icon: (
      <svg className="w-10 h-10 text-persona-management" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Home() {
  return (
    <div data-testid="portal-page" className="min-h-screen bg-surface-page flex flex-col items-center justify-center p-6">
      {/* Tenant Header */}
      <header className="fixed top-0 left-0 right-0 bg-tenant-primary text-white px-4 py-3 flex items-center gap-3 z-50">
        <div className="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center">
          <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
        </div>
        <span className="text-lg font-semibold">Arnfield Care</span>
      </header>
      ...
      <main className="mt-16 flex flex-col items-center">
        ...
        <div className="mb-8 flex items-center gap-3">
          <div className="w-12 h-12 bg-tenant-primary rounded-lg flex items-center justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full mb-12">
          {portals.map((portal) => (
            ...
            >
              <div className="w-16 h-16 rounded-full bg-surface-page flex items-center justify-center mb-4">
                {portal.icon}
              </div>
              ...
              <Link
                href={portal.href}
                className="w-full py-3 px-6 bg-tenant-primary text-white rounded-lg font-medium hover:opacity-90 transition-opacity text-center"
              >
                {portal.linkText}
              </Link>
            </article>
